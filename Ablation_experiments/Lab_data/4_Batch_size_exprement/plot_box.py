...
data = [vector[idx] for idx in batch]
# 繪製箱型圖
colors = sns.color_palette("Set3", 10)  # 一次拿 10 種顏色
plt.figure(figsize=(9, 6))
box = plt.boxplot(data, patch_artist=True, labels=batch)
plt.boxplot(data, labels=batch)
# 給每個 box 上不同顏色
...
plt.title('Comparison of test accuracy for batch size', fontsize=18)
