...
def CM(label_idx, output_idx, CM_path, classes):
    label_flatten = list(chain(*label_idx))
    pred_flatten = list(chain(*output_idx))

    plt.figure(figsize=(10, 8))

    confusion = confusion_matrix(label_flatten, pred_flatten)
    ax = sns.heatmap(
        confusion,
        annot=True,
        fmt='g',
        ...
        xticklabels=classes,
        yticklabels=classes,
        annot_kws={"fontsize": 16}  # heatmap 中的數字字體大小
    )
    ...
    plt.xticks(rotation=45, ha='right')  # 可視情況旋轉
